...
# Define the output directory for the SAMPLE inference results
# Let's use a different output directory for the sample run
OUTPUT_DIR="./inference_output/faster_rcnn_r50_fpn_1x_sample"
...
echo "Running inference with config: $CONFIG_FILE on $MAX_SAMPLES samples"
echo "Saving formatted results to: $OUTPUT_DIR"

python ./test.py ${CONFIG_FILE} \
    --format-only \
    --format-dir ${OUTPUT_DIR} \
    --max-samples ${MAX_SAMPLES} # Add the max samples argument
